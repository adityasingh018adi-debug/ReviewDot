...
export function KpiCard({ kpi, index, live }: { kpi: KpiSeed; index: number; live: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ delay: 0.15 + index * 0.09, type: 'spring', stiffness: 240, damping: 24 }}
    >
      <TiltCard maxTilt={6}>
        <div className="glass group relative overflow-hidden rounded-2xl p-5 shadow-panel transition-colors duration-300 hover:border-white/20">
          <div
            className="absolute -top-10 -right-10 h-28 w-28 rounded-full opacity-25 blur-2xl transition-opacity duration-300 group-hover:opacity-50"

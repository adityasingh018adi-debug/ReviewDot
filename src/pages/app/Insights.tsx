...
import { localAnswer, streamClaude, type AIContext, type ChatMessage } from '@/lib/ai'
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Badge tone={apiKey ? 'positive' : 'neutral'}>{apiKey ? 'Claude connected' : 'Built-in analyst'}</Badge>}
      />
          ...
          <Card>
            ...
            {summary ? (
              <div className="mt-5 space-y-5">
                ...
                <Link
                  to={`/app/products/${summary.productId}`}
      ...
      <Assistant context={context} apiKey={apiKey} />
    </div>
  )
}
...
function Assistant({ context, apiKey }: { context: AIContext; apiKey: string }) {
  ...
  const ask = async (question: string) => {
    ...
    try {
      let streamed = ''
      for await (const chunk of streamClaude(apiKey, context, next)) {
        streamed += chunk
      ...
      setMessages([
        ...
        {
          role: 'assistant',
          content: `Claude could not be reached (${error instanceof Error ? error.message : 'unknown error'}). Falling back to the built-in analyst:\n\n${localAnswer(question, context)}`,
        },
      ])
    } finally {
      setBusy(false)
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle={
          apiKey
            ? 'Answered by Claude, grounded in this workspace’s numbers.'
            : 'Answered by the built-in analyst. Add a Claude API key in Settings for full natural-language answers.'
        }
      />
